...
COURSERA_TOKEN = "w2ASIQghJ1xi35pc"# the key provided to the Student under his/her email on submission page
COURSERA_EMAIL = "tuncutku10@gmail.com"# the email
...
for i in range(len(X)):
    euclidean_distances_vector_l_vectorized.append((((X[i][0]-Y[i][0])**2) + ((X[i][1]-Y[i][1])**2) + ((X[i][2]-Y[i][2])**2) + ((X[i][3]-Y[i][3])**2) + ((X[i][4]-Y[i][4])**2))**(0.5))
