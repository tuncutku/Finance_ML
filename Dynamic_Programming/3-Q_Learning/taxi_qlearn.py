...
MAX_STEPS = env.spec.max_episode_steps  # maximum steps in an episode, 200 for Taxi-v2
...
def update_Q(prev_state, action, reward, cur_state):
    ...
    Q[prev_state][action] += alpha * (
        reward + GAMMA * max(Q[cur_state]) - Q[prev_state][action]
    )
...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODES + 1):
        ...
        for step in range(MAX_STEPS):
            ...
            biggest_change = max(
                biggest_change, np.abs(old_qsa - Q[prev_state][action])
            )
            if done:
                break
